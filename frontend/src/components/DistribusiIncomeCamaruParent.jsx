...
const ORDERED_KATEGORI = ["0", "0–5 jt", "5–10 jt", "10–15 jt", "15–20 jt", ">20 jt"];
...
export default function DistribusiIncomeCamaruParent({ showTable = false, hideHeader = false }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await axios.get("http://127.0.0.1:8000/analytics/parent-distribution/income-camaru");
        const sum = await axios.get("http://127.0.0.1:8000/analytics/parent-distribution/income-camaru-summary");
        setSummary(sum.data);
        ...
        const finalData = Object.values(grouped)
          .map((row) => {
            ORDERED_KATEGORI.forEach((k) => (row[k] ??= 0));
            return row;
          })
  ...
  const filteredSummary = summary.filter(s => Number(s.tahun) >= 2019 && Number(s.tahun) <= 2025);
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      {/* HEADER UTAMA: Hanya muncul jika hideHeader={false} (Bukan di Dashboard) */}
      {!hideHeader && (
        <div style={{ 
          display: "flex", 
          ...
          <div>
            <h2 style={{ fontSize: 24, fontWeight: 800, color: "#0f172a", margin: 0 }}>
              Distribusi Pendapatan Orang Tua Camaru
            </h2>
            <p style={{ fontSize: 16, color: "#475569", margin: "6px 0 0 0" }}>
              Visualisasi pendapatan orang tua calon mahasiswa baru (Data Pendapatan ini telah dilakukan imputasi berdasarkan UMP)
            </p>
          </div>

          <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
            {/* Card Total & Filter: Hanya muncul di halaman utama komponen ini */}
      ...
      {/* HEADER DASHBOARD: Muncul jika hideHeader={true} */}
      {hideHeader && (
        <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 16, color: "#0f172a" }}>
          Distribusi Pendapatan Ortu Camaru
        </h3>
      )}
        ...
        <ResponsiveContainer width="100%" height={hideHeader ? 450 : 500}>
          <BarChart data={data} margin={{ top: 20, right: 30, left: 40, bottom: 60 }}>
            ...
            <Legend 
              ...
              payload={ORDERED_KATEGORI.map((k) => ({ value: k, type: "rect", color: CONTRAST_COLORS[k] }))}
            />

            {ORDERED_KATEGORI.map((k) => (
              <Bar key={k} dataKey={k} stackId="a" fill={CONTRAST_COLORS[k]} barSize={45} />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </div>
